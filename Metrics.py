...
class IED(tf.keras.metrics.Metric):
    def __init__(self, k, name='IED', **kwargs):
    ...
    def update_state(self, y_true, y_pred, sample_weight=None):
        ...
        self.IED = tf.reduce_sum(tf.abs(i_exposure[:, None] - i_exposure[None, :])) / (2. * tf.cast(n_items, tf.float32) * tf.reduce_sum(i_exposure))
